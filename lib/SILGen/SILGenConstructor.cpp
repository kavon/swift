...
void SILGenFunction::emitValueConstructor(ConstructorDecl *ctor) {
  ...
  // Make sure we've hopped to the right actor, if needed.
  // Since this is a value constructor, self is not required because it
  // cannot be isolated to an actor instance.
  if (ctor->hasAsync()) {
    SILLocation prologueLoc(selfDecl);
    prologueLoc.markAsPrologue();
    emitConstructorPrologActorHop(prologueLoc, ctor, selfLV);
  }
...
void SILGenFunction::emitConstructorPrologActorHop(SILLocation loc,
                                                   ConstructorDecl *ctor,
                                                   SILValue self) {
  ...
  switch (isolation.getKind()) {
    ...
    case ActorIsolation::DistributedActorInstance:
      // the initial hop is injected later by definite initialization,
      // but SILGen needs to know that the expected executor is `self`.
      ExpectedExecutor = self;
      return;

    case ActorIsolation::GlobalActor:
    case ActorIsolation::GlobalActorUnsafe:
      // a hop will be injected now in the prolog if global-actor isolated ctor
      // No need to pass self because we know it's a global-actor.
      if (auto executor = emitExecutor(loc, isolation, /*self=*/None)) {
        ExpectedExecutor = *executor;
        B.createHopToExecutor(loc.asAutoGenerated(), *executor,
                              /*mandatory*/ false);
      }
      return;
  }
...
void SILGenFunction::emitClassConstructorInitializer(ConstructorDecl *ctor) {
  ...
  if (ctor->hasAsync()) {
    SILLocation prologueLoc(selfDecl);
    prologueLoc.markAsPrologue();
    emitConstructorPrologActorHop(prologueLoc, ctor, selfArg.getValue());
  }
