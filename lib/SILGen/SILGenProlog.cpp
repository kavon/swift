...
void ExecutorBreadcrumb::emit(SILGenFunction &SGF, SILLocation loc) {
  SILValue exeValue = Executor;

  if (!exeValue)
    return;

  if (auto *project =
          dyn_cast_or_null<ProjectBoxInst>(exeValue.getDefiningInstruction())) {
    auto load = SGF.B.createLoadCopy(loc.asAutoGenerated(),
                                     ManagedValue::forUnmanaged(exeValue));
    
    exeValue = load.getValue();
  }

  SGF.B.createHopToExecutor(loc.asAutoGenerated(), exeValue, /*mandatory*/ false);
}
