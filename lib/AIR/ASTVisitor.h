//===--- ASTVisitor.h - AIR ASTVisitor specialization --------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines swift::air::ASTVisitor.
//
//===----------------------------------------------------------------------===//

#ifndef SWIFT_AIR_ASTVISITOR_H
#define SWIFT_AIR_ASTVISITOR_H
...
/// This is a specialization of swift::ASTVisitor which works only on
/// resolved nodes and which automatically ignores certain AST node kinds.
...
#define UNCHECKED_EXPR(Id, Parent) \
  ExprRetTy visit##Id##Expr(Id##Expr *E, Args...AA) { \
    llvm_unreachable(#Id "Expr should not survive to AIR"); \
  }
#include "swift/AST/ExprNodes.def"
  
  ExprRetTy visitErrorExpr(ErrorExpr *E, Args...AA) {
    llvm_unreachable("expression kind should not survive to AIR");
  }
